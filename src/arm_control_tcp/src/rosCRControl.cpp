...
{
    m_enableRobot = nh->serviceClient<arm_control_tcp::EnableRobot>("/dobot_v4_bringup/srv/EnableRobot");
    m_disableRobot = nh->serviceClient<arm_control_tcp::DisableRobot>("/dobot_v4_bringup/srv/DisableRobot");
    m_clearError = nh->serviceClient<arm_control_tcp::ClearError>("/dobot_v4_bringup/srv/ClearError");
    m_getErrorID = nh->serviceClient<arm_control_tcp::GetErrorID>("/dobot_v4_bringup/srv/GetErrorID");
    m_movj = nh->serviceClient<arm_control_tcp::MovJ>("/dobot_v4_bringup/srv/MovJ");
    m_servoj = nh->serviceClient<arm_control_tcp::ServoJ>("/dobot_v4_bringup/srv/ServoJ");
...
void RosDemoCRV4::getFeedBackInfo(const std_msgs::String::ConstPtr& msg)
{
    ...
    if (parsedJson.count("currentCommandId") && parsedJson["currentCommandId"].is_number()) {
        ...
        // ROS_INFO("feedbackData %d", feedbackData.CurrentCommandID);
    }
}
    ...
    while (true) {
        {
            ...
            if (feedbackData.ErrorStatus) {
                arm_control_tcp::GetErrorID srvGetError;
...
void RosDemoCRV4::moveJoint(std::vector<double>& point, int& currentCommandID)
{
    if (point.size() < 6U) {
        ROS_ERROR("ServoJ params is  ERROR");
        return;
    }

    arm_control_tcp::ServoJ srvServoJ;
    srvServoJ.request.a = point[0];
    srvServoJ.request.b = point[1];
    srvServoJ.request.c = point[2];
    srvServoJ.request.d = point[3];
    srvServoJ.request.e = point[4];
    srvServoJ.request.f = point[5];
    if (!SendService(m_servoj, srvServoJ)) {
        ROS_ERROR("ServoJ service fail");
    } else {
        currentCommandID = int(srvServoJ.response.res);
        ROS_INFO("CurrentCommandID  %d", currentCommandID);
    }
}
...
{
    while (true) {
        // ROS_INFO("CurrentCommandID3  %d", currentCommandID);
        ...
        lockInfo.unlock();
        sleep(0.01);
        // ROS_INFO("CurrentCommandID4  %d", currentCommandID);
    }
}
